...
정직한 정리입니다. "내 환경에서는 돌아간다"는 "실제로 동작한다"와 다릅니다.

**증명됨**
- PoW 솔버는 DeepSeek 자체의 wasm 모듈을 바이트 단위로 재현합니다. 대조 대상은 sha256을 고정한 실제 빌드입니다. 두 백엔드(순수 Python, wasm)를 모두 검증합니다.
- 프로토콜 계약. `tests/test_client_wire.py`가 가짜 전송 계층으로 고정합니다. 브라우저 신원 헤더, impersonation 프로필, 예열 요청, `x-ds-pow-response`의 내용, `/api/v0/chat/completion`으로 보내는 정확한 본문, 그리고 깨진 줄 처리까지 포함한 SSE 파싱.
- 서버, CLI, 설정, 세션 처리. 오프라인 테스트 74개.

**미증명**
- 실제 왕복. 이 프로젝트는 계정을 소유하지 않고, 자격 증명을 포함하지 않으며, 자동 가입이나 CAPTCHA 우회도 넣지 않았습니다. 상류 쪽은 대역 세션으로 검증하므로 확인되는 것은 프로토콜 계약이며, DeepSeek의 실제 응답이 아닙니다.
- 당신의 계정, 할당량, 또는 당신의 네트워크가 거부되는지 여부에 대해서는 아무것도.

이 간극을 메우려면 본인의 세션을 준비하고(위 참조), 첫 요청을 직접 확인하세요.
