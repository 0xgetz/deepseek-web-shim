...
诚实的交代，因为「在我机器上能跑」并不等于「真的可用」。

**已证明**
- PoW 求解器逐字节复现 DeepSeek 自己的 wasm 模块，对照的是 sha256 已固定的真实构建。两种后端（纯 Python、wasm）都经过测试。
- 协议契约，由 `tests/test_client_wire.py` 通过假传输层固定：浏览器身份请求头、impersonation 配置、预热请求、`x-ds-pow-response` 的内容、发送到 `/api/v0/chat/completion` 的确切请求体，以及 SSE 解析（含残缺行的处理）。
- 服务器、CLI、配置与会话处理，共 74 个离线测试。

**尚未证明**
- 真实的往返请求。本项目不拥有任何账号，不附带任何凭据，也不包含自动注册或验证码绕过。上游这一半是对着替身会话测试的，因此验证的是协议契约，而不是 DeepSeek 的真实回复。
- 关于你的账号、配额，或者你的网络是否会被拒绝的任何事。

要补上这段差距，请提供你自己的会话（见上文），并亲自观察第一个请求。
