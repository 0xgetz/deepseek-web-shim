<div align="center">
  ...
  <p>
    ...
    <a href="tests"><img src="https://img.shields.io/badge/tests-74%20offline-2ea44f?style=flat-square" alt="54 件のオフラインテスト"></a>
  </p>
  <p>
...
## 証明済みのこと、まだ証明されていないこと

正直な内訳です。「自分の環境では動く」は「実際に機能する」と同じではありません。

**証明済み**
- PoW ソルバは DeepSeek 自身の wasm モジュールをバイト単位で再現します。照合先は sha256 を固定した実物のビルドです。両方のバックエンド（純 Python、wasm）を検証しています。
- プロトコル契約。`tests/test_client_wire.py` が偽のトランスポートで固定します。ブラウザ識別ヘッダ、impersonation プロファイル、ウォームアップ要求、`x-ds-pow-response` の中身、`/api/v0/chat/completion` に送られる正確なボディ、そして壊れた行の扱いを含む SSE の解析。
- サーバ、CLI、設定、セッション処理。オフラインテスト 74 件。

**未証明**
- 実際の往復。本プロジェクトはアカウントを保有せず、認証情報を同梱せず、自動登録や CAPTCHA 回避も含みません。上流側は代役セッションに対して検証しているため、確認できるのはプロトコル契約であり、DeepSeek からの実際の返信ではありません。
- あなたのアカウント、クォータ、あるいはあなたの回線が拒否されるかどうかについては何も。

この差を埋めるには、ご自身のセッションを用意し（上記参照）、最初のリクエストを自分の目で確認してください。
